...
def test_system_design_workflow_e2e(workflow_path):
    ...
    def mock_verify_hypotheses(hypotheses, answers, config):
        return {
            "is_valid": True, 
            "best_hypothesis": "H1", 
    ...
    def mock_generate_solution(hypothesis, draft, is_valid, config):
        return {"solution": "Solution"}

    def mock_critic_review(solution, is_valid, config):
        return {"final_solution": "Final Solution"}
    
    def mock_determine_next_state(retry_input, next_input, next_action, is_valid, config):
        return {
            "should_stop": True, 
            "next_question": ""
        }

    def mock_save_results(solution, config):
    ...
    mock_functions = {
        ...
        "ask_user_next_steps": lambda solution, is_valid, config: {},
